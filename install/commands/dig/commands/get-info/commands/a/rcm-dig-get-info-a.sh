...
usage() {
    cat << 'EOF'
Usage: rcm dig get-info a [options] [ip-address]

ip-address:
    Set the IP Address of A record to be match.
...
isRecordExist() {
    ...
    local name="$3"
    local data="$4"
    [ -n "$5" ] && tempfile="$5" || tempfile=$(mktemp -p /dev/shm -t rcm-dig-get-info-a.XXXXXX)
    if [ "$data" == '@' ];then
        data="$domain"
    fi
...
if [ -z "$ip_address" ];then
    code dig A "$fqdn_string" $add_name_server +short
    dig A "$fqdn_string" $add_name_server +short
    exit 0
fi
