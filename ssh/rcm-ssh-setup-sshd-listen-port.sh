...
# Functions.
[[ $(type -t RcmSshSetupSshdListenPort_printVersion) == function ]] || RcmSshSetupSshdListenPort_printVersion() {
    echo '0.1.0'
}
[[ $(type -t RcmSshSetupSshdListenPort_printHelp) == function ]] || RcmSshSetupSshdListenPort_printHelp() {
    cat << EOF
RCM SSH Setup
Variation SSHD Listen Port
Version `RcmSshSetupSshdListenPort_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmSshSetupSshdListenPort_printHelp; exit 1; }
[ -n "$version" ] && { RcmSshSetupSshdListenPort_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmSshSetupSshdListenPort_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmSshSetupSshdListenPort_printVersion`; _.
____
