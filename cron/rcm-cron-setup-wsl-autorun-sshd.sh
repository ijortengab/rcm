...
# Functions.
[[ $(type -t RcmCronSetupWslAutorunSshd_printVersion) == function ]] || RcmCronSetupWslAutorunSshd_printVersion() {
    echo '0.1.0'
}
[[ $(type -t RcmCronSetupWslAutorunSshd_printHelp) == function ]] || RcmCronSetupWslAutorunSshd_printHelp() {
    cat << EOF
RCM Cron Setup
Variation WSL Autorun SSHD
Version `RcmCronSetupWslAutorunSshd_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmCronSetupWslAutorunSshd_printHelp; exit 1; }
[ -n "$version" ] && { RcmCronSetupWslAutorunSshd_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmCronSetupWslAutorunSshd_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmCronSetupWslAutorunSshd_printVersion`; _.
____
