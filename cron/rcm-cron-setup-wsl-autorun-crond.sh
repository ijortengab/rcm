...
# Functions.
[[ $(type -t RcmCronSetupWslAutorunCrond_printVersion) == function ]] || RcmCronSetupWslAutorunCrond_printVersion() {
    echo '0.1.0'
}
[[ $(type -t RcmCronSetupWslAutorunCrond_printHelp) == function ]] || RcmCronSetupWslAutorunCrond_printHelp() {
    cat << EOF
RCM Cron Setup
Variation WSL Autorun CROND
Version `RcmCronSetupWslAutorunCrond_printVersion`

EOF
    cat << 'EOF'
Usage: rcm-cron-setup-wsl-autorun-crond.sh
...
# Help and Version.
[ -n "$help" ] && { RcmCronSetupWslAutorunCrond_printHelp; exit 1; }
[ -n "$version" ] && { RcmCronSetupWslAutorunCrond_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmCronSetupWslAutorunCrond_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmCronSetupWslAutorunCrond_printVersion`; _.
____
