...
# Functions.
[[ $(type -t RcmCronSetupWslPortForwarding_printVersion) == function ]] || RcmCronSetupWslPortForwarding_printVersion() {
    echo '0.1.0'
}
[[ $(type -t RcmCronSetupWslPortForwarding_printHelp) == function ]] || RcmCronSetupWslPortForwarding_printHelp() {
    cat << EOF
RCM Cron Setup
Variation WSL Port Forwarding
Version `RcmCronSetupWslPortForwarding_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmCronSetupWslPortForwarding_printHelp; exit 1; }
[ -n "$version" ] && { RcmCronSetupWslPortForwarding_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmCronSetupWslPortForwarding_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmCronSetupWslPortForwarding_printVersion`; _.
____
