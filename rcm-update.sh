...
if ! command -v $rcm_extension >/dev/null;then
    error "Command not found: ${rcm_extension}."; x
fi
...
blob_path="$path"
backup_path="${HOME}/.cache/rcm/${github_owner_repo}/rollback/${rcm_extension}"
current_version=`$rcm_extension --version`
current_path=$(which $rcm_extension)
current_path=$(realpath "$current_path")
____

if [ -n "$rollback" ];then
    chapter Rolling back.
    if [ -f "$backup_path" ];then
        ...
        current_version=`$rcm_extension --version`
        _ 'Success rollback '; magenta $rcm_extension; _, ' to version: '; yellow $current_version; _.
        rm "$backup_path"
        # Dalam satu direktori ini bisa terdapat banyak file. Contoh
        # rcm, rcm-composer-autoinstaller
        rmdir --ignore-fail-on-non-empty "${HOME}/.cache/rcm/${github_owner_repo}/rollback"
    fi
    ____
...
if [ "$version" == latest ];then
    ...
    if [ "$current_version" == "$version" ];then
        if [ -n "$loud" ];then
            _ 'You are already using the latest available '; magenta $rcm_extension; _, ' version: '; yellow $version; _.
        fi
        exit 0
    fi
fi
...
current_version=`$rcm_extension --version`
if [ -n "$loud" ];then
    _ 'Success update '; magenta $rcm_extension; _, ' to version: '; yellow $current_version; _.
    _ To rollback version $old_version, execute the latest command with --rollback options.; _.
fi
____

command="${rcm_extension}"
Rcm_event_dispatcher 'Post Update'
