...
# Functions.
[[ $(type -t RcmNginxSetupStatic_printVersion) == function ]] || RcmNginxSetupStatic_printVersion() {
    echo '0.1.0'
}
[[ $(type -t RcmNginxSetupStatic_printHelp) == function ]] || RcmNginxSetupStatic_printHelp() {
    cat << EOF
RCM Nginx Setup
Variation Static
Version `RcmNginxSetupStatic_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmNginxSetupStatic_printHelp; exit 1; }
[ -n "$version" ] && { RcmNginxSetupStatic_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmNginxSetupStatic_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmNginxSetupStatic_printVersion`; _.
____
