...
# Functions.
[[ $(type -t RcmNginxSetupPhpFpm_printVersion) == function ]] || RcmNginxSetupPhpFpm_printVersion() {
    echo '0.1.0'
}
[[ $(type -t RcmNginxSetupPhpFpm_printHelp) == function ]] || RcmNginxSetupPhpFpm_printHelp() {
    cat << EOF
RCM Nginx Setup
Variation PHP FPM
Version `RcmNginxSetupPhpFpm_printVersion`

EOF
    cat << 'EOF'
Usage: rcm-nginx-setup-php-fpm.sh [options]
...
# Help and Version.
[ -n "$help" ] && { RcmNginxSetupPhpFpm_printHelp; exit 1; }
[ -n "$version" ] && { RcmNginxSetupPhpFpm_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmNginxSetupPhpFpm_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmNginxSetupPhpFpm_printVersion`; _.
____
