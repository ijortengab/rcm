...
Rcm_prompt() {
    ...
    if [ -n "$value" ];then
        ...
        subcommand="$value"
        options=`$rcm_extension $subcommand --help 2>/dev/null | sed -n '/^Options for command '$subcommand'[:\.]$/,$p' | sed -n '1,/^\s*$/p' | sed -n '2,/^\s*$/p'`
    else
        options=`$rcm_extension --help 2>/dev/null | sed -n '/^Options[:\.]$/,$p' | sed -n '1,/^\s*$/p' | sed -n '2,/^\s*$/p'`
    fi
        ...
        until [[ -z "$options" ]];do
            ...
            if [[ -z "$options" && -z "$load_other_options" ]];then
                if [ -n "$subcommand" ];then
                    other_options=`$rcm_extension --help 2>/dev/null | sed -n '/^Other [Oo]ptions for command '$subcommand'.*[:\.]$/,$p' | sed -n '1,/^\s*$/p' | sed -n '2,/^\s*$/p'`
                else
                    other_options=`$rcm_extension --help 2>/dev/null | sed -n '/^Other [Oo]ptions.*[:\.]$/,$p' | sed -n '1,/^\s*$/p' | sed -n '2,/^\s*$/p'`
                fi
...
Rcm_event_dispatcher() {
    ...
    to_execute=`${rcm_extension} --help 2>/dev/null | sed -n '/^'"$label"'[:\.]$/,$p' | sed -n '1,/^\s*$/p' | sed -n '2,/^\s*$/p'`
...
command -v "$rcm_extension" >/dev/null || { red "Unable to proceed, $rcm_extension command not found."; x; }
...
backup_storage=$HOME'/.cache/rcm/rcm.'$rcm_extension'.bak'
history_storage=$HOME'/.cache/rcm/rcm.'$rcm_extension'.history'
trap Rcm_prompt_sigint SIGINT
Rcm_prompt $rcm_extension
trap x SIGINT
...
words_array=(${RCM_ENVIRONMENT_VARIABLES} ${rcm_extension} ${isfast} ${isverbose} $@)
wordWrapCommand
____
...
if [ -n "$confirmation" ];then
    ...
    _ Direct to '`'${rcm_extension}'`' command.; _.
    ____
...
INDENT+="$RCM_INDENT" BINARY_DIRECTORY="$BINARY_DIRECTORY" $rcm_extension $isfast $isverbose "$@"
