...
Version `RcmDrupalSetupVariation1_printVersion`

EOF
    cat << 'EOF'
Usage: rcm-drupal-setup-variation1.sh [options]
...
# Help and Version.
[ -n "$help" ] && { RcmDrupalSetupVariation1_printHelp; exit 1; }
[ -n "$version" ] && { RcmDrupalSetupVariation1_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmDrupalSetupVariation1_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmDrupalSetupVariation1_printVersion`; _.
____
