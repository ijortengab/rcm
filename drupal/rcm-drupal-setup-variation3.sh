...
# Functions.
[[ $(type -t RcmDrupalSetupVariation3_printVersion) == function ]] || RcmDrupalSetupVariation3_printVersion() {
    echo '0.1.1'
}
[[ $(type -t RcmDrupalSetupVariation3_printHelp) == function ]] || RcmDrupalSetupVariation3_printHelp() {
    cat << EOF
RCM Drupal Setup
Variation 3. Ubuntu 22.04, Drupal 10, PHP 8.2.
Version `RcmDrupalSetupVariation3_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmDrupalSetupVariation3_printHelp; exit 1; }
[ -n "$version" ] && { RcmDrupalSetupVariation3_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmDrupalSetupVariation3_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmDrupalSetupVariation3_printVersion`; _.
____
