...
# Functions.
[[ $(type -t RcmIspconfigSetupInternalCommand_printVersion) == function ]] || RcmIspconfigSetupInternalCommand_printVersion() {
    echo '0.1.0'
}
[[ $(type -t RcmIspconfigSetupInternalCommand_printHelp) == function ]] || RcmIspconfigSetupInternalCommand_printHelp() {
    cat << EOF
RCM ISPConfig Setup
Variation Internal Command
Version `RcmIspconfigSetupInternalCommand_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmIspconfigSetupInternalCommand_printHelp; exit 1; }
[ -n "$version" ] && { RcmIspconfigSetupInternalCommand_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmIspconfigSetupInternalCommand_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmIspconfigSetupInternalCommand_printVersion`; _.
____
