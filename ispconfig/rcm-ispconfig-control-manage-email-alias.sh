...
Version `RcmIspconfigControlManageEmailAlias_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmIspconfigControlManageEmailAlias_printHelp; exit 1; }
[ -n "$version" ] && { RcmIspconfigControlManageEmailAlias_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmIspconfigControlManageEmailAlias_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
[[ $(type -t getMailUserIdIspconfigByEmail) == function ]] || getMailUserIdIspconfigByEmail() {
...
[[ $(type -t isEmailIspconfigExist) == function ]] || isEmailIspconfigExist() {
...
[[ $(type -t mailboxCredential) == function ]] || mailboxCredential() {
    local host="$1"
    local user="$2"
...
[[ $(type -t insertEmailIspconfig) == function ]] || insertEmailIspconfig() {
...
[[ $(type -t getUserIdRoundcubeByUsername) == function ]] || getUserIdRoundcubeByUsername() {
...
[[ $(type -t isUsernameRoundcubeExist) == function ]] || isUsernameRoundcubeExist() {
...
[[ $(type -t insertUsernameRoundcube) == function ]] || insertUsernameRoundcube() {
...
[[ $(type -t getIdentityIdRoundcubeByEmail) == function ]] || getIdentityIdRoundcubeByEmail() {
    # Get the user_id from table users in roundcube database.
    #
    # Globals:
    #   roundcube_db_user, roundcube_db_user_password,
...
[[ $(type -t isIdentitiesRoundcubeExist) == function ]] || isIdentitiesRoundcubeExist() {
    # Check if the username from table users exists in roundcube database.
    #
    # Globals:
    #   Used: roundcube_db_user, roundcube_db_user_password,
...
[[ $(type -t insertIdentitiesRoundcube) == function ]] || insertIdentitiesRoundcube() {
    # Insert the username to table users in roundcube database.
    #
    # Globals:
    #   Used: roundcube_db_user, roundcube_db_user_password,
...
[[ $(type -t getForwardingIdIspconfigByEmailAlias) == function ]] || getForwardingIdIspconfigByEmailAlias() {
...
[[ $(type -t isEmailAliasIspconfigExist) == function ]] || isEmailAliasIspconfigExist() {
...
[[ $(type -t insertEmailAliasIspconfig) == function ]] || insertEmailAliasIspconfig() {
...
_ 'Version '; yellow `RcmIspconfigControlManageEmailAlias_printVersion`; _.
____
...
until [[ -n "$destination_name" ]];do
    read -p "Argument --destination-name required: " destination_name
done
code 'destination_name="'$destination_name'"'
until [[ -n "$destination_domain" ]];do
    read -p "Argument --destination-domain required: " destination_domain
done
