...
Version `RcmIspconfigAutoinstallerNginxPhpFpm_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmIspconfigAutoinstallerNginxPhpFpm_printHelp; exit 1; }
[ -n "$version" ] && { RcmIspconfigAutoinstallerNginxPhpFpm_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmIspconfigAutoinstallerNginxPhpFpm_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
[[ $(type -t toggleMysqlRootPassword) == function ]] || toggleMysqlRootPassword() {
...
_ 'Version '; yellow `RcmIspconfigAutoinstallerNginxPhpFpm_printVersion`; _.
____
