...
# Functions.
[[ $(type -t RcmMariadbSetupDatabase_printVersion) == function ]] || RcmMariadbSetupDatabase_printVersion() {
    echo '0.1.0'
}
[[ $(type -t RcmMariadbSetupDatabase_printHelp) == function ]] || RcmMariadbSetupDatabase_printHelp() {
    cat << EOF
RCM MariaDB Setup Database
Variation Default
Version `RcmMariadbSetupDatabase_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmMariadbSetupDatabase_printHelp; exit 1; }
[ -n "$version" ] && { RcmMariadbSetupDatabase_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmMariadbSetupDatabase_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmMariadbSetupDatabase_printVersion`; _.
____
