...
# Functions.
[[ $(type -t RcmMariadbSetupIspconfig_printVersion) == function ]] || RcmMariadbSetupIspconfig_printVersion() {
    echo '0.1.0'
}
[[ $(type -t RcmMariadbSetupIspconfig_printHelp) == function ]] || RcmMariadbSetupIspconfig_printHelp() {
    cat << EOF
RCM MariaDB Setup
Variation ISPConfig
Version `RcmMariadbSetupIspconfig_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmMariadbSetupIspconfig_printHelp; exit 1; }
[ -n "$version" ] && { RcmMariadbSetupIspconfig_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmMariadbSetupIspconfig_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmMariadbSetupIspconfig_printVersion`; _.
____
