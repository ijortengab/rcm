...
# Functions.
[[ $(type -t RcmPhpAutoinstaller_printVersion) == function ]] || RcmPhpAutoinstaller_printVersion() {
    echo '0.1.0'
}
[[ $(type -t RcmPhpAutoinstaller_printHelp) == function ]] || RcmPhpAutoinstaller_printHelp() {
    cat << EOF
RCM PHP Auto-Installer
Variation Default
Version `RcmPhpAutoinstaller_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmPhpAutoinstaller_printHelp; exit 1; }
[ -n "$version" ] && { RcmPhpAutoinstaller_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmPhpAutoinstaller_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmPhpAutoinstaller_printVersion`; _.
____
