...
# Functions.
[[ $(type -t RcmPhpmyadminAutoinstallerNginxPhpFpm_printVersion) == function ]] || RcmPhpmyadminAutoinstallerNginxPhpFpm_printVersion() {
    echo '0.1.2'
}
[[ $(type -t RcmPhpmyadminAutoinstallerNginxPhpFpm_printHelp) == function ]] || RcmPhpmyadminAutoinstallerNginxPhpFpm_printHelp() {
    cat << EOF
RCM PHPMyAdmin Auto-Installer
Variation Nginx PHP-FPM
Version `RcmPhpmyadminAutoinstallerNginxPhpFpm_printVersion`
...
# Help and Version.
[ -n "$help" ] && { RcmPhpmyadminAutoinstallerNginxPhpFpm_printHelp; exit 1; }
[ -n "$version" ] && { RcmPhpmyadminAutoinstallerNginxPhpFpm_printVersion; exit 1; }

# Dependency.
while IFS= read -r line; do
    command -v "${line}" >/dev/null || { echo -e "\e[91m""Unable to proceed, ${line} command not found." "\e[39m"; exit 1; }
done <<< `RcmPhpmyadminAutoinstallerNginxPhpFpm_printHelp | sed -n '/^Dependency:/,$p' | sed -n '2,/^$/p' | sed 's/^ *//g'`
...
_ 'Version '; yellow `RcmPhpmyadminAutoinstallerNginxPhpFpm_printVersion`; _.
____
